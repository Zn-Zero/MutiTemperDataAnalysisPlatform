...
function requireLoadBindings() {
  ...
  var __importDefault = loadBindings && loadBindings.__importDefault || function(mod) {
    return mod && mod.__esModule ? mod : { "default": mod };
  };
...
function initSerialIpc(mainWindow) {
  _mainWindow = mainWindow;
  ipcMain.handle("serial:list", async () => {
    try {
      const ports = await distExports.SerialPort.list();
      return ports.map((p) => ({
        path: p.path,
  ...
  ipcMain.handle("serial:open", async (_, options) => {
    ...
    return new Promise((resolve, reject) => {
      _port = new distExports.SerialPort({ ...options, autoOpen: false });
      _port.on("open", () => {
  ...
  ipcMain.handle("serial:write", async (_, data, encoding = "utf8") => {
    return new Promise((resolve, reject) => {
      ...
      _port.write(writeData, (err) => {
        ...
        _port.drain((err2) => {
          if (err2) console.warn("[数据发送警告]", err2.message);
